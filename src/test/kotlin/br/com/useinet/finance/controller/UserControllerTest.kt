...
class UserControllerTest {
    ...
    @Test
    fun me_shouldReturnAuthenticatedUserProfile() {
        val response = userController.me(usuario)
        assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
        assertThat(response.body!!.id).isEqualTo(1L)
        assertThat(response.body!!.nome).isEqualTo("Carlos")
        assertThat(response.body!!.email).isEqualTo("carlos@example.com")
    }

    @Test
    fun atualizar_shouldUpdateNome() {
        `when`(usuarioRepository.save(usuario)).thenReturn(usuario)
        val response = userController.atualizar(usuario, UpdateProfileRequest(nome = "Carlos Novo"))
        assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
        assertThat(response.body!!.nome).isEqualTo("Carlos Novo")
        verify(usuarioRepository).save(usuario)
    }

    @Test
    fun atualizar_shouldTrimNome() {
        `when`(usuarioRepository.save(usuario)).thenReturn(usuario)
        userController.atualizar(usuario, UpdateProfileRequest(nome = "  Carlos  "))
        assertThat(usuario.nome).isEqualTo("Carlos")
    }
    ...
    @Test
    fun atualizar_shouldThrowWhenNomeIsBlank() {
        assertThatThrownBy { userController.atualizar(usuario, UpdateProfileRequest(nome = "   ")) }
            .isInstanceOf(IllegalArgumentException::class.java)
            .hasMessage("Nome não pode ser vazio.")
    }
